...
class TestDatabaseManager:
    ...
    Для каждого теста создаётся новый in-memory движок SQLite,
    и таблицы создаются заново (drop_all вызывается перед create_all),
    что предотвращает конфликты с индексами.
    """
    def __init__(self):
        self.engine = None
...
# Фикстура для API-клиента с использованием ASGITransport; используем shared_db для переопределения зависимости get_session
@pytest.fixture
