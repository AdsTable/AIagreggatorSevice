...
class TestDatabaseManager:
    """Manages test database lifecycle with better isolation"""
    ...
    async def setup_for_class(self, class_name):
        ...
        # Create a unique database for each test class
        # Use in-memory database for speed but with unique connection string
        db_url = f"sqlite+aiosqlite:///:memory:{uuid.uuid4()}"
        
        # Or use temporary file for more persistence during debugging
        # temp_file = tempfile.NamedTemporaryFile(suffix='.db', delete=False)
        # temp_file.close()
        # self.temp_files[class_name] = temp_file.name
    ...
    async def clear_data(self, class_name):
        """Clear data in a specific test class database"""
    ...
    async def cleanup_for_class(self, class_name):
        ...
        if class_name in self.engines:
            ...
            # If using temporary files, remove them
            if class_name in self.temp_files:
                file_path = self.temp_files[class_name]
                if os.path.exists(file_path):
                    os.unlink(file_path)
                del self.temp_files[class_name]
...
if __name__ == "__main__":
    ...
    print("pytest improved_test_api_v2.py -v --asyncio-mode=auto")
    print()
    print("To run specific test categories:")
    print("pytest improved_test_api_v2.py::TestDatabase -v --asyncio-mode=auto")
    print("pytest improved_test_api_v2.py::TestSearchAndFilter -v --asyncio-mode=auto")
    print("pytest improved_test_api_v2.py::TestAPI -v --asyncio-mode=auto")