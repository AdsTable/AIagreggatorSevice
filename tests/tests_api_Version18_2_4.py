...
class TestDatabaseManager:
    ...
    Используется in‑memory SQLite, при инициализации базы вызывается drop_all перед create_all,
    чтобы устранить ошибку "index ix_productdb_category already exists".
    """
    def __init__(self):
        self.engine = None
    ...
    async def setup(self):
        ...
        async with self.engine.begin() as conn:
            # Сбрасываем все таблицы перед созданием новых
...
# Фикстура для API-клиента с использованием ASGITransport для передачи приложения
@pytest.fixture
...
class TestDataParserFunctions:
    ...
    def test_parse_availability(self):
        # Проверяем, что функция возвращает bool для разных вариантов входа
