...
class DatabaseManager:
    """Manager for database operations in tests."""
...
# --- Session Provider for Tests ---
class TestSessionProvider:
    """Session provider that emulates FastAPI dependency behavior."""
    
    def __init__(self, session: AsyncSession):
        self.session = session
    
    async def __call__(self) -> AsyncGenerator[AsyncSession, None]:
        """Return session as an async generator to match FastAPI's dependency behavior."""
        yield self.session
...
async def setup_database():
    """Setup database before tests and cleanup after."""
...
async def session():
    ...
    try:
        # Provide session to the test
        yield session
    finally:
        # Always rollback transaction to keep tests isolated
...
async def client(session: AsyncSession):
    ...
    # Create a provider that will yield our test session
    provider = TestSessionProvider(session)
    
    # Override get_session dependency with our provider
