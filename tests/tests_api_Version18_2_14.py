...
class TestDatabaseManager:
    """Manages test database lifecycle with file-based SQLite"""
    
    def __init__(self):
        self.engines = {}
        self.session_factories = {}
        self.db_files = {}  # Tracks DB file paths for cleanup
    
    async def setup_for_class(self, class_name):
        """Setup a new database file for a specific test class"""
        # Create a unique temporary file for this test class
        temp_dir = tempfile.gettempdir()
        unique_id = str(uuid.uuid4())[:8]  # Use short UUID to avoid path length issues
        db_filename = f"test_db_{class_name}_{unique_id}.sqlite"
        db_path = os.path.join(temp_dir, db_filename)
        
        # Store for later cleanup
        self.db_files[class_name] = db_path
        
        # Create SQLite connection URL (use file protocol)
        # Using aiosqlite adapter for async operations
        ...
        # Explicitly drop all tables before creating to avoid index conflicts
    ...
    async def clear_data(self, class_name):
        """Clear data in a specific test class database without dropping schema"""
        if class_name in self.engines:
            async with self.engines[class_name].begin() as conn:
                # DELETE is safer than dropping tables between tests
    ...
    async def cleanup_for_class(self, class_name):
        ...
        if class_name in self.engines:
            ...
            if class_name in self.db_files:
                db_path = self.db_files[class_name]
                try:
...
async def class_db_setup(request):
    ...
    # Explicitly setup database for this class
...
async def db_session(request, class_db_setup):
    ...
    # Clear data before each test to start with clean state
    ...
    try:
        ...
        # Make sure to close the session after test
        await session.close()
...
if __name__ == "__main__":
    ...
    print("pytest improved_test_api_file_db.py -v --asyncio-mode=auto")
    print()
    print("To run specific test categories:")
    print("pytest improved_test_api_file_db.py::TestDatabase -v --asyncio-mode=auto")
    print("pytest improved_test_api_file_db.py::TestSearchAndFilter -v --asyncio-mode=auto")
    print("pytest improved_test_api_file_db.py::TestAPI -v --asyncio-mode=auto")