...
class StandardizedProduct:
    ...
    def __eq__(self, other) -> bool:
        """Custom equality that matches the hash logic"""
...
class TestDataIntegrity:
    ...
    def test_product_hashing(self, optimized_test_products):
        """Test that products can be properly hashed with correct equality"""
        product = optimized_test_products[0]
        ...
        # Create a product with same identity fields but different other fields
        product2 = StandardizedProduct(
            source_url=product.source_url,      # Same identity field
            category=product.category,          # Same identity field  
            name=product.name,                  # Same identity field
            provider_name=product.provider_name, # Same identity field
            product_id=product.product_id,      # Same identity field
        ...
        # Products should be equal based on identity fields
        assert product == product2, "Products with same identity should be equal"
        
        # Same hash value
        assert hash(product) == hash(product2), f"Equal products must have same hash"
        
        # Can be used in sets - should be treated as same item
        product_set = {product, product2}
        assert len(product_set) == 1, f"Set should contain 1 item, got {len(product_set)}"
        
        # Test different products have different identity
        different_product = StandardizedProduct(
            source_url="https://different.com",  # Different identity field
            category=product.category,
            name=product.name,
        ...
        # Should not be equal
        assert product != different_product, "Products with different identity should not be equal"
        
        # Different products in set
        product_set_with_different = {product, different_product}
        assert len(product_set_with_different) == 2, "Different products should create set of size 2"
