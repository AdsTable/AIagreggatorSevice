...
class TestDatabaseManager:
    ...
    Теперь для каждого теста создаётся новый движок in-memory SQLite,
    и таблицы создаются заново (drop_all вызывается при setup).
    Это решает проблему конфликта индексов, таких как ix_productdb_category.
    """
    def __init__(self):
        self.engine = None
    ...
    async def setup(self):
        ...
        async with self.engine.begin() as conn:
            # Сбрасываем таблицы (если они существуют) и создаём заново
...
# Для каждого теста будем создавать новую базу
...
# Фикстура для API-клиента с использованием ASGITransport
@pytest.fixture
async def api_client():
    # Переопределение зависимости get_session в FastAPI через асинхронный генератор
...
class TestAPI:
    ...
    async def test_search_endpoint_no_filters(self, api_client, test_products):
        session = (await get_session())  # получаем отдельную сессию для сохранения данных
