...
async def db_session(setup_test_environment): # Ensure setup_test_environment runs first
    ...
    try:
        ...
        await session.close()
        # await test_db.cleanup() # Cleanup database AFTER each test - Moved to test function itself
...
class TestAPI:
    ...
    def test_search_endpoint_no_filters(self, api_client, test_products):  # Inject db_session
        """Test search endpoint without filters"""
        # Setup data first
        with test_db.session_factory() as session: # Use context manager
            try:
                asyncio.run(store_standardized_data(session=session, data=test_products))
            except Exception as e:
    ...
    def test_search_endpoint_with_filters(self, api_client, test_products): # Inject db_session
