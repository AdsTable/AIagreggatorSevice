...
# Custom dependency for tests that will use our test session
async def override_get_session():
    db_session = pytest._fixture_db_session  # Retrieve session from pytest global state
    try:
        yield db_session
    finally:
        pass  # No cleanup needed here, session is managed by the fixture

@pytest.fixture(scope="function")
async def client(db_session):
    """Test client fixture."""
    # Store the session in pytest for retrieval in override_get_session
    pytest._fixture_db_session = db_session
    ...
    if hasattr(pytest, "_fixture_db_session"):
        delattr(pytest, "_fixture_db_session")
