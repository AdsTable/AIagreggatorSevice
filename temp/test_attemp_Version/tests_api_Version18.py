...
class TestIntegration:
    ...
    async def test_full_workflow_mobile_plans(self, db_session):
        ...
        # Test cost filtering
        budget_plans = await search_and_filter_products(
            session=db_session,
            product_type="mobile_plan",
            max_price=30  # Using max_price even though it's monthly_cost
        )
        # This might not work as expected since we're filtering by price_kwh
        # but it tests the filter system
    ...
    async def test_full_workflow_internet_plans(self, db_session):
        ...
        high_speed = await search_and_filter_products(
            session=db_session,
            product_type="internet_plan",
...
class TestEdgeCases:
    ...
    async def test_large_dataset_performance(self, db_session):
        ...
        filtered_results = await search_and_filter_products(
            session=db_session,
            provider="Provider 5",
            available_only=True,
            min_price=0.15
...
class TestAPIErrors:
    ...
    async def test_api_malformed_query_parameters(self, api_client):
        ...
        # Test with non-numeric values for numeric parameters
        response = await api_client.get("/search?min_price=not_a_number")
        # FastAPI should handle validation, but shouldn't crash
        # Response could be 400 (validation error) or 200 (ignored parameter)
