...
class TestDatabaseManager:
    """Manages test database lifecycle with better isolation using temporary files"""
    ...
    async def setup_for_class(self, class_name):
        """Setup a new database for a specific test class using a temporary file"""
        # Создаем уникальный файл для каждого класса тестов
        db_file = os.path.join(self.temp_dir, f"{class_name}_{uuid.uuid4().hex}.db")
        self.db_files[class_name] = db_file
        
        # Использование aiosqlite с файловой БД
        ...
        # Критически важная часть: удаляем ВСЕ таблицы, а затем создаем заново
    ...
    async def cleanup_all(self):
        ...
        try:
            if os.path.exists(self.temp_dir):
                # Убедимся, что все файлы удалены
...
if __name__ == "__main__":
    ...
    print("pytest improved_test_api_final.py -v --asyncio-mode=auto")
    print()
    print("To run specific test categories:")
    print("pytest improved_test_api_final.py::TestDatabase -v --asyncio-mode=auto")
    print("pytest improved_test_api_final.py::TestSearchAndFilter -v --asyncio-mode=auto")
    print("pytest improved_test_api_final.py::TestAPI -v --asyncio-mode=auto")