...
import uuid
import logging
import datetime  # Added this import for the datetime functions
...
def json_serialize_safe(obj: Any) -> Union[str, float, int, bool, None]:
    ...
    if isinstance(obj, float) and (obj == float('inf') or obj == float('-inf') or obj != obj):  # last check for NaN
        return "unlimited" if obj == float('inf') else str(obj)
    return obj
    ...
    **kwargs
    ...
    for db_product in filtered_db_products:
        ...
        product_dict = {
            ...
            "calls": "unlimited" if db_product.calls == float('inf') else 
                     float(db_product.calls) if db_product.calls is not None else None,
            "texts": "unlimited" if db_product.texts == float('inf') else 
                     float(db_product.texts) if db_product.texts is not None else None,
            ...
            "data_cap_gb": "unlimited" if db_product.data_cap_gb == float('inf') else 
                           float(db_product.data_cap_gb) if db_product.data_cap_gb is not None else None,
...
def test_products():
    ...
    return [
        StandardizedProduct(
            ...
            product_id="elec-001",  # Added product_id
            price_kwh=0.15,
            standing_charge=5.0,
        ...
        StandardizedProduct(
            ...
            product_id="elec-002",  # Added product_id
            price_kwh=0.12,
            standing_charge=4.0,
        ...
        StandardizedProduct(
            ...
            name="Mobile Plan C",
            provider_name="Provider X",
            product_id="mobile-001",  # Added product_id
            monthly_cost=30.0,
            data_gb=100.0,
            calls=float("inf"),
            texts=float("inf"),
...
async def api_client():
    ...
    """
    # Set up patching for the search_and_filter_products function
    # This function should be patched in the module where API routes are defined
    search_patch_path = 'main.search_and_filter_products'
    
    with patch(search_patch_path, side_effect=mock_search_and_filter_products):
...
class TestDataParserFunctions:
    ...
    def test_json_serialize_safe(self):
        ...
        assert json_serialize_safe(float('inf')) == "unlimited"
        assert json_serialize_safe(float('-inf')) == "-inf"
        
        # Test NaN handling
        assert json_serialize_safe(float('nan')) == "nan"
...
class TestSearchAndFilter:
    ...
    async def test_search_all_products(self, mock_db_session, test_products):
        ...
        assert mobile_product["calls"] == "unlimited"
        assert mobile_product["texts"] == "unlimited"
...
class TestAPI:
    ...
    async def test_search_endpoint_with_data(self, api_client, test_products):
        ...
        data = response.json()
        assert isinstance(data, list)
        # Now that we've populated the mock database, we should get results
        assert len(data) == len(test_products)
        
        # Verify the JSON response can be properly deserialized
        # This confirms our infinity handling works correctly
        for product in data:
            if product.get("name") == "Mobile Plan C":
                assert product.get("calls") == "unlimited"
                assert product.get("texts") == "unlimited"
...
if __name__ == "__main__":
    ...
    print("✅ JSON-compliant serialization for special values (infinity)")
    print()
    ...
    print("- Semantically meaningful handling of special values")