# test_api_final.py - Окончательное решение для FastAPI async тестирования
import pytest
import sys
import os
import json
...
# === ПРОСТОЕ И ЭФФЕКТИВНОЕ РЕШЕНИЕ 2025 ===
...
async def create_test_session() -> AsyncSession:
    """Создать тестовую сессию БД - БЕЗ YIELD!"""
    engine, factory = await get_test_engine()
    return factory()

async def create_isolated_session() -> AsyncSession:
    """Создать изолированную сессию с транзакцией."""
    session = await create_test_session()
    # Начинаем транзакцию, которую потом откатим
    await session.begin()
    return session
...
# === DATABASE TESTS (ПРЯМОЕ СОЗДАНИЕ СЕССИЙ) ===
...
@pytest.mark.asyncio
async def test_search_endpoint_filter_by_type(api_client, test_products):
...
@pytest.mark.asyncio
async def test_search_endpoint_filter_by_min_price(api_client, test_products):
    """Test API filtering by min price."""
    session = await create_test_session()
    try:
        await store_standardized_data(session=session, data=test_products)
    finally:
...
@pytest.mark.asyncio
async def test_search_endpoint_filter_by_provider(api_client, test_products):
...
@pytest.mark.asyncio
async def test_search_endpoint_filter_by_max_data_gb(api_client, test_products):
...
@pytest.mark.asyncio
async def test_search_endpoint_filter_by_min_contract_duration(api_client, test_products):
...
@pytest.mark.asyncio
async def test_search_endpoint_combined_filters(api_client, test_products):
    """Test API with combined filters."""
    session = await create_test_session()
    try:
        await store_standardized_data(session=session, data=test_products)
    finally:
...
@pytest.mark.asyncio
async def test_search_endpoint_no_results(api_client, test_products):
    """Test API with no results."""
    session = await create_test_session()
    try:
        await store_standardized_data(session=session, data=test_products)
    finally:
...
@pytest.mark.asyncio
async def test_search_endpoint_all_filters(api_client, test_products):
