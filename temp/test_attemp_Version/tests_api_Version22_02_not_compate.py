"""
PRODUCTION JSON-SAFE TEST SUITE - COMPLETE OPTIMIZED VERSION
...
class TestEdgeCasesAndJsonSafety:
        ...
        StandardizedProduct(
            ...
            raw_data={
                ...
                "nested": {
                    ...
                    "list_with_inf": [1.0, float('inf'), float('nan'), 3.0]
                }
            }
        ),
        StandardizedProduct(
    ...
    optimized_mock_db.add_products(edge_case_products)
    ...
    product1 = results[0]
    assert product1['price_kwh'] == JSON_SAFE_POSITIVE_INF
    assert product1['standing_charge'] == JSON_SAFE_NEGATIVE_INF  
    assert product1['monthly_cost'] is None  # NaN -> None
    assert product1['data_gb'] == 0.0        # Normal value preserved
    assert product1['calls'] == -1.0         # Negative value preserved
    
    # Проверяем санитизацию raw_data
    raw_data1 = product1['raw_data']
    assert raw_data1['pos_inf'] == JSON_SAFE_POSITIVE_INF
    assert raw_data1['neg_inf'] == JSON_SAFE_NEGATIVE_INF
    assert raw_data1['nan_val'] is None
    assert raw_data1['normal'] == 42.5
    
    # Проверяем глубокую вложенную санитизацию
    nested = raw_data1['nested']
    assert nested['inf_nested'] == JSON_SAFE_POSITIVE_INF
    assert nested['list_with_inf'][0] == 1.0
    assert nested['list_with_inf'][1] == JSON_SAFE_POSITIVE_INF
    assert nested['list_with_inf'][2] is None  # NaN -> None
    assert nested['list_with_inf'][3] == 3.0
    
    # Проверяем второй продукт  
    product2 = results[1]
    assert product2['download_speed'] == JSON_SAFE_POSITIVE_INF
    assert product2['upload_speed'] == JSON_SAFE_POSITIVE_INF
    assert product2['data_cap_gb'] is None    # NaN -> None
    assert product2['texts'] == JSON_SAFE_NEGATIVE_INF
    
    # Проверяем сложную вложенную структуру
    complex_nested = product2['raw_data']['complex_nested']
    assert complex_nested['level1']['level2']['inf_deep'] == JSON_SAFE_POSITIVE_INF
    values = complex_nested['level1']['level2']['values']
    assert values[0] is None  # NaN -> None
    assert values[1] == JSON_SAFE_POSITIVE_INF
    assert values[2] == JSON_SAFE_NEGATIVE_INF
    
    # Проверяем смешанный массив объектов
    mixed_array = product2['raw_data']['mixed_array']
    assert mixed_array[0]['value'] == JSON_SAFE_POSITIVE_INF
    assert mixed_array[1]['value'] is None  # NaN -> None  
    assert mixed_array[2]['value'] == 123.45
    
    # ГЛАВНАЯ ПРОВЕРКА: JSON сериализация должна работать БЕЗ ОШИБОК
    try:
        # Тест с кастомным энкодером
        json_str1 = safe_json_dumps(results)
        assert isinstance(json_str1, str)
        assert len(json_str1) > 100
        assert 'inf' not in json_str1.lower()
        assert 'nan' not in json_str1.lower()
        
        # Тест со стандартным json модулем (должен работать т.к. данные уже санитизированы)
        ...
        assert len(parsed_data1) == 2
        assert len(parsed_data2) == 2
        
        print("✅ Test 14 passed: Comprehensive JSON edge cases - ПОЛНОСТЬЮ ИСПРАВЛЕНО")
        
    except ValueError as e:
        if "Out of range float values are not JSON compliant" in str(e):
            pytest.fail(f"❌ JSON serialization failed with infinity/NaN error: {e}")
        else:
            raise e
    except Exception as e:
        pytest.fail(f"❌ Unexpected error during JSON processing: {e}")