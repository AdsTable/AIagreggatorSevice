"""
tests_api_Version22_03.py::TestEdgeCasesAndJsonSafety::test_infinity_values_json_safe FAILED 
tests_api_Version22_03.py::TestEdgeCasesAndJsonSafety::test_large_dataset_json_safe_performance FAILED
WARNING  AIagreggatorSevice.tests.tests_api_Version22_03:tests_api_Version22_03.py:694 Skipping product Performance Test Product 99 - failed JSON safety validation
=============================================================== short test summary info =============================================================== 
FAILED tests_api_Version22_03.py::TestEdgeCasesAndJsonSafety::test_infinity_values_json_safe - assert 'inf' not in '[{"source_u...an":null}}}]'
FAILED tests_api_Version22_03.py::TestEdgeCasesAndJsonSafety::test_large_dataset_json_safe_performance - assert 0 > 0
"""
...
def safe_json_dumps(obj: Any, **kwargs) -> str:
    ...
    try:
        kwargs['cls'] = EnhancedJsonSafeEncoder
...
class TestEnhancedBasicFunctionality:
    ...
    def test_store_and_retrieve_json_safe(self):
        ...
        assert str(1e308) not in json_str
        try:
            json.loads(json_str)
    ...
    async def test_async_store_and_retrieve_json_safe(self):
        ...
        assert stored_count == len([p for p in test_products if p.validate_json_safety()])
        ...
        json_str = safe_json_dumps(results)
        assert isinstance(json_str, str)
        assert 'inf' not in json_str.lower().replace(str(JsonSafeConfig.POSITIVE_INF_REPLACEMENT).lower(), '')
        assert 'nan' not in json_str.lower().replace('null', '')  # if NAN_STRATEGY == 'null'
...
class TestEdgeCasesAndJsonSafety:
    ...
    def test_infinity_values_json_safe(self):
        ...
        # Проверка NaN конвертации
        if JsonSafeConfig.NAN_STRATEGY == 'null':
            assert product.get('monthly_cost') is None
        ...
        # Проверка статистики конвертации
        stats = conversion_stats.get_stats()
        assert stats['total'] >= 6  # Минимум 6 конвертаций (inf, -inf, nan)
