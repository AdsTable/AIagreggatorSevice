...
# version -1 Claude-Copilot - 07.06.25
...
try:
    ...
    logging.warning("Performance monitor not available, using fallback")
    ...
    class PerformanceMonitor:
        ...
        async def monitor_operation(self, operation_name: str, operation_type: OperationType = OperationType.COMPUTATION):
            yield None
...
# Import business logic modules with comprehensive error handling
try:
...
try:
    ...
    logging.warning(f"Database functions import failed: {e}")
...
try:
    ...
    logging.warning(f"Data processing modules import failed: {e}")
...
class AppConfig:
    ...
    enable_docs: bool = os.getenv('ENABLE_DOCS', 'true').lower() == 'true'  # New: explicit docs control
...
# Initialize metrics and monitor
metrics_collector = Metrics(app_name="AI Aggregator Pro")

if 'PerformanceMonitor' not in globals():
    class PerformanceMonitor:
...
app = FastAPI(
    ...
    lifespan=lifespan,
    docs_url="/docs" if config.enable_docs else None,  # Fixed: conditional docs enabling
    redoc_url="/redoc" if config.enable_docs else None,
    openapi_url="/openapi.json" if config.enable_docs else None  # Fixed: conditional OpenAPI
)
...
@app.post("/ai/ask", summary="Ask AI", description="Submit a question to AI with smart provider selection")
@rate_limit(50)
...
async def root():
    ...
    return {
        ...
        "documentation_note": "Visit /docs for interactive API documentation" if config.enable_docs else "Documentation disabled"
    }
